...
import { Check, Plus, Minus, Star, ArrowRight, X, CreditCard, User, Mail, Phone, MapPin } from 'lucide-react';
...
const faqs = [
  { q: "Is there a cancellation fee?", a: "No. We believe in earning your business every month. You can cancel anytime with 30 days notice." },
  { q: "Can I freeze my membership?", a: "Yes, you can freeze your membership for up to 3 months per year for medical or travel reasons." },
  { q: "Are classes included in the Day Pass?", a: "Day passes grant access to the gym floor only. Classes require a Pro membership or a drop-in fee." },
  { q: "Do you offer student discounts?", a: "Yes! Students with a valid ID get 15% off the Pro Membership." }
];
...
const Pricing: React.FC = () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    alert(`Membership selected: ${selectedPlan?.name}\nProceeding to Esewa payment...`);
    ...
    <>
            ...
            >
              <div className="p-8">
                ...
                <div className="bg-slate-50 rounded-2xl p-6 mb-8">
                  <h3 className="text-sm font-black text-slate-700 uppercase tracking-wider mb-4">Plan Includes</h3>
                ...
                <form onSubmit={handleSubmit} className="space-y-5">
                  ...
                  <div className="pt-6 border-t-2 border-slate-200">
                    <h3 className="text-sm font-black text-slate-700 uppercase tracking-wider mb-4">Payment Method</h3>
                    <div className="bg-gradient-to-br from-purple-50 to-pink-50 border-2 border-purple-200 rounded-xl p-4 flex items-center gap-4">
                      ...
                      <div className="flex-grow">
                        <p className="font-black text-slate-900">Esewa Payment</p>
        ...
        <div className="text-center mb-12 max-w-7xl mx-auto">
          ...
          >
            <div className="inline-block mb-4">
              <span className="px-4 py-2 bg-primary/10 text-primary rounded-full text-sm font-bold uppercase tracking-wider">Choose Your Plan</span>
            </div>
            <h2 className="text-4xl md:text-5xl font-black text-slate-900 mb-4">
              Flexible <span className="text-transparent bg-clip-text bg-gradient-to-r from-primary to-emerald-500">Memberships</span>
            </h2>
            <p className="text-slate-600 text-lg max-w-2xl mx-auto">Invest in yourself with flexible plans tailored to your fitness goals and lifestyle.</p>
          </motion.div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-6xl mx-auto">
          {plans.map((plan, idx) => (
            <motion.div 
              ...
              viewport={{ once: true }}
              className={`relative p-8 rounded-3xl border flex flex-col ${plan.recommended ? 'bg-gradient-to-br from-primary to-emerald-500 border-primary shadow-2xl scale-105 z-10' : 'bg-white border-slate-200 hover:border-primary transition-all hover:-translate-y-2 hover:shadow-xl'}`}
            >
              {plan.recommended && (
              ...
              <h3 className={`text-xl font-black uppercase tracking-wider mb-2 ${plan.recommended ? 'text-white/90' : 'text-slate-600'}`}>{plan.name}</h3>
              <div className="flex items-baseline mb-8">
                <span className={`text-5xl font-black ${plan.recommended ? 'text-white' : 'text-slate-900'}`}>{plan.price}</span>
                <span className={`ml-2 font-semibold ${plan.recommended ? 'text-white/90' : 'text-slate-500'}`}>/ month</span>
              </div>
              
              <ul className="space-y-4 mb-8 flex-grow">
                {plan.features.map((feature, idx) => (
                  <li key={idx} className="flex items-start gap-3">
                    <Check className={`w-5 h-5 shrink-0 ${plan.recommended ? 'text-white' : 'text-primary'}`} />
                    <span className={`text-sm font-medium ${plan.recommended ? 'text-white/95' : 'text-slate-600'}`}>{feature}</span>
                  </li>
                ))}
              </ul>
              ...
              <Button 
                ...
                className={`w-full justify-center ${plan.recommended ? 'bg-white text-primary hover:bg-gray-50 shadow-lg' : 'bg-primary hover:bg-primary/90 shadow-lg shadow-primary/20'}`}
      ...
      <Section className="bg-gradient-to-b from-slate-50 to-white py-16 relative overflow-hidden">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 max-w-7xl mx-auto relative z-10">
           ...
           >
             <div className="inline-block mb-4">
               <span className="px-4 py-2 bg-primary/10 text-primary rounded-full text-sm font-bold uppercase tracking-wider">Need Help?</span>
             </div>
             <h2 className="text-4xl md:text-5xl font-black text-slate-900 mb-6">
               Frequently Asked <span className="text-transparent bg-clip-text bg-gradient-to-r from-primary to-emerald-500">Questions</span>
             </h2>
             <p className="text-slate-600 text-lg leading-relaxed mb-8">Can't find what you're looking for? Contact our support team and we'll be happy to help.</p>
             <Link to="/contact">
               <Button variant="primary" className="gap-2 bg-primary hover:bg-primary/90 shadow-lg shadow-primary/20">Contact Support <ArrowRight /></Button>
             </Link>
           </motion.div>
        ...
        >
          <div className="bg-gradient-to-br from-primary via-emerald-500 to-primary rounded-3xl p-12 md:p-16 shadow-2xl relative overflow-hidden">
            ...
            <div className="text-center relative z-10">
              <h2 className="text-3xl md:text-5xl font-black text-white mb-6">Start Your Free 7-Day Trial</h2>
              <p className="text-white/95 text-xl leading-relaxed mb-8 max-w-2xl mx-auto">
                Experience everything NeonFit has to offer with no commitment required.
              </p>
              <Link to="/contact">
                <Button variant="secondary" className="bg-white text-primary hover:bg-gray-50 shadow-xl">Get Started Now</Button>
              </Link>
            </div>
          </div>
        </motion.div>
