...
type NotificationItem = {
  ...
  title: string;
  description: string;
  tone: 'product' | 'order' | 'message' | 'membership';
  source?: 'product' | 'order' | 'message' | 'membership';
  sourceId?: number;
};
...
const getAuthUser = (): AdminProfile | null => {
...
const bucketByMonth = (orders: OrderRecord[]) => {
...
const bucketOrderStatuses = (orders: OrderRecord[]) => {
...
const bucketProductsByCategory = (products: ProductRecord[]) => {
  const bucket = new Map<string, number>();
...
const buildLinePath = (values: number[], width = 560, height = 220, padding = 24) => {
  if (!values.length) {
    return '';
  }
...
const Dashboard: React.FC = () => {
  ...
  const [adminProfile, setAdminProfile] = useState<AdminProfile | null>(getAuthUser());
  const [products, setProducts] = useState<ProductRecord[]>([]);
  const [orders, setOrders] = useState<OrderRecord[]>([]);
  const [messages, setMessages] = useState<MessageRecord[]>([]);
  const [services, setServices] = useState<Array<{ id: number }>>([]);
  const [categories, setCategories] = useState<Array<{ id: number }>>([]);
  ...
  const [dismissedNotificationIds, setDismissedNotificationIds] = useState<string[]>(() => getStoredIds('adminDismissedNotificationIds'));
  const [readNotificationIds, setReadNotificationIds] = useState<string[]>(() => getStoredIds('adminReadNotificationIds'));
  ...
  useEffect(() => {
    const loadDashboard = async () => {
      ...
      const unwrap = <T,>(result: PromiseSettledResult<T>) =>
        result.status === 'fulfilled' ? result.value : null;

      const productPayload = unwrap(productResult as PromiseSettledResult<any>);
      const orderPayload = unwrap(orderResult as PromiseSettledResult<any>);
      const messagePayload = unwrap(messageResult as PromiseSettledResult<any>);
      const servicePayload = unwrap(serviceResult as PromiseSettledResult<any>);
      const categoryPayload = unwrap(categoryResult as PromiseSettledResult<any>);
      const profilePayload = unwrap(profileResult as PromiseSettledResult<any>);
  ...
  const activitySeries = useMemo(() => {
    const bucket = new Map<string, { label: string; orders: number; products: number; messages: number }>();

    const bump = (dateValue: string | undefined, key: 'orders' | 'products' | 'messages') => {
      if (!dateValue) return;
  ...
  const statusToneClass = (tone: NotificationItem['tone']) => {
    switch (tone) {
      case 'product':
