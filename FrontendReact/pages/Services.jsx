...
import { Wifi, Coffee, Car, Lock, ShowerHead, Music, ArrowRight, CheckCircle2 } from 'lucide-react';
          ...
          {loading ? (
            ...
            services.map((s, idx) => (
              <motion.div 
                ...
                viewport={{ once: true }}
                className="relative bg-white rounded-3xl overflow-hidden border border-slate-200 hover:border-primary transition-all hover:-translate-y-2 hover:shadow-2xl group"
              >
                <div className="relative h-64 overflow-hidden">
                  <img 
                    src={s.photo_path ? `http://localhost:8000/storage/${s.photo_path}` : 'https://images.unsplash.com/photo-1534438327276-14e5300c3a48?w=600&h=400&fit=crop'} 
                    alt={s.name} 
                    className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500"
                  />
                  <div className="absolute inset-0 bg-gradient-to-t from-slate-900/80 via-slate-900/30 to-transparent"></div>
                  <div className="absolute bottom-0 left-0 right-0 p-6">
                    <h3 className="text-2xl font-black text-white leading-tight">{s.name}</h3>
                  </div>
                </div>
                <div className="p-6">
                  <p className="text-slate-600 leading-relaxed line-clamp-3">{s.description}</p>
                  <Link to={`/service/${s.id}`} className="inline-flex items-center gap-2 mt-4 text-primary font-bold hover:gap-3 transition-all">
